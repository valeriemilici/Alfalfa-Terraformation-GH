...
1. C = no plants, inoculated with microbes

2. N = no plants, no microbe inoculation
...
Ok, so we can see that soil moisture does affect porewater pH, which peaks at intermediate soil moisture. We can also see some marginal effects of soil. Treatments N and S have lower pHs than L pots, but this is super marginal. This implies that the presence of plants and microbes together leads to higher weathering (weathering will increase pH in basalt). If you focus on the std. error around the estimate, you can see that live and sterile soil treatments (aka basalt that contains vascular plants) has a small std. error around the estimate (~ 0.06 in both cases), while both bare soil treatments have large std. error (~ 0.14). The model is struggling to estimate what the pH is for those bare soil treatments, compared to the planted treatments. It may be that plants make some values, such as pH more predictable/deterministic. The [variance tests](#b-testpH) tell us that planted treatments have significantly less variation than bare soil treatments. 

This might have to do with the number of samples in each treatment. There are fewer bare samples than live samples. Investigate this further on Monday.
...
```{r EC Soil, message = F}
ECSoil <- lmer(log(EC) ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)
```
...
pred.funpH <- function(mod) {
  model.matrix(~Soil,
               data = expand.grid(Soil =c("L","C", "N", "S"))) %*%
    fixef(mod)[c(1:4)]
}

pred.funEC <- function(mod) {
  model.matrix(~plant,
               data = expand.grid(plant =c("N", "Y"))) %*%
    fixef(mod)[c(1:2)]
}
...
ECsims <- bootMer(ECSoil, FUN = pred.funEC,
                nsim = 1000, parallel = "snow", ncpus = detectCores(),cl = cl)

pHsims <- bootMer(pHSoil, FUN = pred.funpH,
                nsim = 1000, parallel = "snow", ncpus = detectCores(),cl = cl)

stopCluster(cl = cl)

#Prepare Data for Bartlett Test
ECdraws <- data.frame(ECsims$t)
pHdraws <- data.frame(pHsims$t)
...
pHdraws2 <- pivot_longer(pHdraws, cols = L:S)
...
bartlett.test(list(pHdraws$L, pHdraws$C))
```

yes.
```{r pH mod F test}
var.test(pHdraws$L, pHdraws$C)

var.test(pHdraws$S, pHdraws$N)
```
...
ECdraws2 <- pivot_longer(ECdraws, cols = N:Y)

densityPlot(exp(ECdraws2$value) ~ as.factor(ECdraws2$name),
            legend = list(title = "with plant"),
...
bartlett.test(list(exp(ECdraws$N), exp(ECdraws$Y)))
```
According to Bartlett test, the variances are different.

```{r EC mod  F-test}
var.test(ECdraws$N, ECdraws$Y)
```
...
TCSoil2 <- lmer(TC_ppm ~ soil_LSB * Level + scale(Dmass_tot) +as.factor(batch_TC) +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

TCSoil3 <- lmer(TC_ppm ~ plant_YN * Level + scale(Dmass_tot) +as.factor(batch_TC) +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

compare_performance(TCSoil1, TCSoil2, TCSoil3, rank = T)
...
ICSoil2 <- lmer(IC ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_IC),
                data = dat)

ICSoil3 <- lmer(IC ~ plant_YN * Level + scale(Dmass_tot)  +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_IC),
                data = dat)

compare_performance(ICSoil1, ICSoil2, ICSoil3, rank = T)
...
In the absence of microbes, the S and N treatments have lower IC than the treatment with microbes and plants (L). No other comparisons are significant. IC is something that we expect as a product of weathering- bicarbonates and stuff. Overall, at high soil moisture there is less IC. Perhaps this is because the pore spaces were saturated and it could have limited reaction rates. There is something interesting once more with the abiotic treatment, N (no microbes, no plants), IC increases quite a bit as soil moisture increases. This suggests that the reaction rates weren't limited under high soil moisture, but for some other reason. 
...
OCSoil2 <- lmer(NPOC ~ soil_LSB + Level + scale(Dmass_tot) + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

OCSoil3 <- lmer(NPOC ~ plant_YN + Level + scale(Dmass_tot)  + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

compare_performance(OCSoil1, OCSoil2, OCSoil3, rank = T)

anova(OCSoil2)
```

The best model will combine N and C treatments as a single factor (B = bare soil), without an interaction between soil treatment and moisture level.

```{r echo = F}
dat$soil_LSB <- relevel(factor(dat$soil_LSB), 
                     ref = "L")
```

```{r OC, message = F}

OCSoil <- lmer(log(NPOC) ~ soil_LSB + Level + scale(Dmass_tot) + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)
```
...
NO3Soil2 <- lmer(mu_NO3 ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = dat)

NO3Soil3 <- lmer(mu_NO3 ~ plant_YN  * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = dat)

compare_performance(NO3Soil1, NO3Soil2, NO3Soil3, rank = T)

#Step 2:
NO3Soil3b <- lmer(mu_NO3 ~ plant_YN  + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = dat)

compare_performance(NO3Soil3, NO3Soil3b, rank = T)
```

The best model only considers whether or not there was a plant in the pot, but no interaction with soil moisture. Some of the very low readings for Nitrate were reducing model performance, so they have been removed. 


```{r Nitrate Soil Model, message=F}
NO3Soil <- lmer(log(mu_NO3) ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
...
NH4Soil2 <- lmer(log(mu_NH4) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table)+ (1|batch_anion),
                data = dat)

NH4Soil3 <- lmer(log(mu_NH4) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table)+ (1|batch_anion),
                data = dat)

compare_performance(NH4Soil1, NH4Soil2, NH4Soil3, rank = T)

NH4Soil2b <- lmer(log(mu_NH4) ~ soil_LSB + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table)+ (1|batch_anion),
                data = dat)

compare_performance(NH4Soil2, NH4Soil2b, rank = T)
```

The best model considers whether or not plants had microbes against bare soil treatments together and includes an interaction with soil moisture.  
...
NH4Soil <- lmer(log(mu_NH4) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table)+ (1|batch_anion),
                data = dat)
```
...
NaSoil2 <- lmer(log(mu_Na) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

NaSoil3 <- lmer(log(mu_Na) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(NaSoil1, NaSoil2, NaSoil3, rank = T)

NaSoil1b <- lmer(log(mu_Na) ~ Soil + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(NaSoil1, NaSoil1b, rank = T)
```

The best model includes all soil treatments and an interaction between soil and level

```{r Sodium Soil Model, message=F}
NaSoil <- lmer(log(mu_Na) ~ Soil * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)
```
...
CaSoil2 <- lmer(log(mu_Ca) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = filter(dat, mu_Ca >= quantile(mu_Ca, na.rm = T, 0.2)))

CaSoil3 <- lmer(log(mu_Ca) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = filter(dat, mu_Ca >= quantile(mu_Ca, na.rm = T, 0.2)))

compare_performance(CaSoil1, CaSoil2, CaSoil3, rank = T)

CaSoil3b <- lmer(log(mu_Ca) ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = filter(dat, mu_Ca >= quantile(mu_Ca, na.rm = T, 0.2)))

compare_performance(CaSoil3, CaSoil3b, rank = T)
```
...
CaSoil <- lmer(log(mu_Ca) ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = filter(dat, mu_Ca >= quantile(mu_Ca, na.rm = T, 0.2)))
```
...
MgSoil2 <- lmer(log(mu_Mg) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

MgSoil3 <- lmer(log(mu_Mg) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(MgSoil1, MgSoil2, MgSoil3, rank = T)

MgSoil3b <- lmer(log(mu_Mg) ~ soil_LSB + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(MgSoil3, MgSoil3b, rank = T)
```

The best model only evaluates bare vs live or sterile planted, and does not include an interaction with moisture
...
Magnesium does reveal a pattern. I also reverted the model to a form that isn't the absolute best because if I want to use this model in a manuscript, I feel like the "LSB" treatment designation is a bit confusing. 
...
KSoil2 <- lmer(log(mu_K) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

KSoil3 <- lmer(log(mu_K) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(KSoil1, KSoil2, KSoil3, rank = T)

# Step 2: Should treatment and level interact?

KSoil1b <- lmer(log(mu_Mg) ~ Soil + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cat),
                data = dat)

compare_performance(KSoil1, KSoil1b, rank = T)
```
...
The most robust pattern is that Sodium and Potassium (and Inorganic Carbon) concentrations peak at intermediate soil moisture, with the exception of the "N" treatment (abiotic; no microbes, no plants) where concentrations increase as soil moisture increases. Does this tell us something about how biota affect the rates that these things are produced? 
...
FSoil2 <- lmer(log(mu_F) ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_F <= quantile(mu_F, na.rm = T, 0.85)))

FSoil3 <- lmer(log(mu_F) ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_F <= quantile(mu_F, na.rm = T, 0.85)))

compare_performance(FSoil1, FSoil2, FSoil3, rank = T)

# Step 2: Should treatment and level interact?

FSoil2b <- lmer(log(mu_F) ~ soil_LSB + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_F <= quantile(mu_F, na.rm = T, 0.85)))

compare_performance(FSoil2, FSoil2b, rank = T)
```

The best model combines both bare soil treatments and does not interact with soil moisture. The data between quantiles 0 - 0.85 is 2 - 35, and jumps to 100 at quantile 0.9 and is 435 at quantile 1, so I removed these extreme values that were creating a problem in the data. I have looked at the pots that produced the very high values and they are all planted treatments with an even mix between L and S treatments. When I do include these values in the model, the only difference is that the pattern with soil moisture becomes slightly weaker (and the model fits much worse).

These really high values of F are all from the same two batches; 1 & 2. I think the machine gave bad values on those two days. All other anions are within their normal range in batches 1 and 2, so this issue is for fluoride only. 
...
FSoil <- lmer(log(mu_F) ~ soil_LSB + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_F <= quantile(mu_F, na.rm = T, 0.85)))
```
...
BrSoil2 <- lmer(mu_Br ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Br <= quantile(mu_Br, na.rm = T, 0.9)))

BrSoil3 <- lmer(mu_Br ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Br <= quantile(mu_Br, na.rm = T, 0.9)))

compare_performance(BrSoil1, BrSoil2, BrSoil3, rank = T)

# Step 2: Should treatment and level interact?

BrSoil3b <- lmer(mu_Br ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Br <= quantile(mu_Br, na.rm = T, 0.9)))

compare_performance(BrSoil3, BrSoil3b, rank = T)
```
...
BrSoil <- lmer(mu_Br ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Br <= quantile(mu_Br, na.rm = T, 0.9)))
```
...
ClSoil2 <- lmer(mu_Cl ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Cl <= quantile(mu_Cl, na.rm = T, 0.95)))

ClSoil3 <- lmer(mu_Cl ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Cl <= quantile(mu_Cl, na.rm = T, 0.95)))

compare_performance(ClSoil1, ClSoil2, ClSoil3, rank = T)

# Step 2: Should treatment and level interact?

ClSoil3b <- lmer(mu_Cl ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Cl <= quantile(mu_Cl, na.rm = T, 0.95)))

compare_performance(ClSoil3, ClSoil3b, rank = T)
```
...
ClSoil <- lmer(mu_Cl ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_Cl <= quantile(mu_Cl, na.rm = T, 0.95)))
```
...
PO4Soil2 <- lmer(mu_PO4 ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_PO4 <= 25))

PO4Soil3 <- lmer(mu_PO4 ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_PO4 <= 25))

compare_performance(PO4Soil1, PO4Soil2, PO4Soil3, rank = T)

# Step 2: Should treatment and level interact?

PO4Soil1b <- lmer(mu_PO4 ~ Soil + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_PO4 <= 25))


compare_performance(PO4Soil1, PO4Soil1b, rank = T)
```

The best model includes all soil treatments with an interaction with level. There are some outlier values that I remove to improve model fit. 
...
The abiotic treatment, N, has the highest phosphate levels, and phosphate only increases as soil moisture increases. With the addition of biota, we see a decrease in phosphate. The presence of plants seems to have a stronger effect on the draw down (use) of phosphate than microbes alone. We can see that the S and L treatments are significantly lower than N, whle the C treatment is only marginally lower. 

You will notice the absence of the C treatment at higher soil moisture and that is because none of the samples produced values. There are also very few N samples that produced readings of PO4 in the data, which makes me suspicious of this model. Take this relationship with a grain of salt. I am much more likely to use a model looking at variation among genotypes because we have plenty of readings from the planted treatments. 
...
SO4Soil2 <- lmer(mu_SO4 ~ soil_LSB * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_SO4 <= 4000))

SO4Soil3 <- lmer(mu_SO4 ~ plant_YN * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_SO4 <= 4000))

compare_performance(SO4Soil1, SO4Soil2, SO4Soil3, rank = T)

# Step 2: Should treatment and level interact?

SO4Soil3b <- lmer(mu_SO4 ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_SO4 <= 4000))


compare_performance(SO4Soil3, SO4Soil3b, rank = T)
```

The best model only compares whether there was a plant and has no interaction

```{r SO4 Soil Model, message=F}
SO4Soil <- lmer(mu_SO4 ~ plant_YN + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_anion),
                data = filter(dat, mu_SO4 <= 4000))
summary(SO4Soil)
```
...
dat1 <- dat %>% dplyr::select(1,9,22:34, 36, 40, 42,43) 

colSums(is.na(dat1))
#remove NO2, PO4, NH4

dat2 <- dat1 %>% dplyr::select(!c(mu_NO2, mu_PO4, mu_NH4, mu_Li, mu_Br, mu_NO3)) %>%
  filter(!is.na(mu_F), !is.na(mu_Cl), !is.na(mu_SO4),
         !is.na(pH), !is.na(EC), !is.na(mu_Na), !is.na(mu_K),
         !is.na(mu_Mg), !is.na(mu_Ca), !is.na(TC_ppm), !is.na(IC)
         )

colSums(is.na(dat2)) 

dat3 <- dat2 %>% dplyr::select(!c(pot, Soil)) #now only numbers for PCA
...
pWGeochem <- fviz_pca_biplot(pwGeo_pca, 
                col.ind = dat2$Soil, palette = "jco", 
                addEllipses = T, ellipse.level = 0.95, label = "var", repel = TRUE,
                legend.title = "Soil Treatment") +
  theme_classic(12)

pWGeochem
```

So... not much going on here. There's no separation of the different soil treatments based on the 11 porewater geochemistry variables that I included. I wonder what this would look like if the plants had been larger! The PCA includes 11 variables shared between 85 observations (out of a maximum of 97). 
