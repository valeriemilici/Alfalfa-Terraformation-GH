...
### Total Cations / IC (another general measure of weathering)

```{r}
dat2 <- dat %>% mutate(CatIC = mu_charge / (IC/12)) %>%
  filter(CatIC >= 0 & CatIC != Inf)
```


```{r Total Cation / IC Soil model selection, include=F }

CatICSoil2 <- lmer(CatIC ~ Soil * Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cation) + (1|batch_IC),
                data = dat2)
```

Microbes at high soil moisture have a higher Cation/IC ratio than the other treatments. This is the only significant relationship (other than accounting for dilution).

```{r echo=F, message=F}
CatICSoil <- lmer(mu_charge ~ IC + Soil + Level + scale(Dmass_tot) +
                 (1|Geno) + (1|dis_table) + (1|table) + (1|batch_cation) + (1|batch_IC),
                data = dat)
```
...
check_model(CatICSoil)
```
looks good to me!

```{r echo=F}
CatICSoil %>% tidy() %>% filter(!is.na(p.value)) %>% dplyr::select(-c(1:2)) %>% kable(digits = 5)
```

The only thing that affects cation concentration is IC, not treatment 

```{r}

quantile(dat$mu_charge, na.rm = T, 0.975) #148 is the high
quantile(dat$mu_charge, na.rm = T, 0.025) #0 is the low

library(ggpmisc)
catICfig <- ggplot(filter(dat, mu_charge <=148), aes(mu_charge, IC/12)) +
  stat_poly_line() +
  stat_poly_eq(use_label(c("R2", "eq"))) +
  geom_point() +
  xlab("Total Cation Concentration (ppm)") +
  ylab("IC Concentration (ppm)") +
  theme_bw()

ggsave(filename = "~/Desktop/Alfalfa-Terraformation-GH/Task2A/04_analyses/04_Figures/ManuscriptFigs/catICfig.png", plot = catICfig)
```
...
### Total Nitrogen (measured alongside NPOC)
...
```{r Nitrogen Soil model selection, include=F }

#Step 1:
NSoil1 <- lmer((TN.ppm) ~ Soil * Level + scale(Dmass_tot) + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

NSoil2 <- lmer((TN.ppm) ~ Soil + Level + scale(Dmass_tot) + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = dat)

compare_performance(NSoil1, NSoil2, rank = T)
```
...
```{r TN Soil Model, message=F}
NSoil <- lmer(log(TN.ppm) ~ Soil + Level + scale(Dmass_tot) + batch_NPOC +
                 (1|Geno) + (1|dis_table) + (1|table),
                data = filter(dat, TN.ppm <= 55 & TN.ppm >= 1.26))
#remove outlier that is above 0.975 quantile
```

*Check Model Diagnostics*

```{r TN Diagnostics, echo=FALSE, fig.dim=c(6,8)}
check_model(NSoil)
```


```{r TN Model Summary, echo=FALSE}
NSoil %>% tidy() %>% filter(!is.na(p.value)) %>% dplyr::select(-c(1:2)) %>% kable(digits = 5)
```

There are absolutely no patterns with total discharge N and any of the treatments (for both additive and interactive models)

```{r TN model save, echo = FALSE}
saveRDS(NSoil, "~/Desktop/Alfalfa-Terraformation-GH/Task2A/04_analyses/02_models/Output/NSoil.RDS")
```


```{r TN vs NPOC plot, echo = FALSE}
NCarbon <- lmer((TN.ppm) ~ NPOC + scale(Dmass_tot) + batch_NPOC + 
                 (1|Geno) + (1|dis_table) + (1|table),
                data = filter(dat, TN.ppm <= 55 & TN.ppm >= 1.26))
```

```{r NCarbon plot, message=F, echo=FALSE}
NCpreds <- predict_response(NCarbon, terms = "NPOC", type = "fixed")

# Create Plot

NCplot <- ggplot(data = NCpreds, mapping = aes(x, predicted)) +
   geom_ribbon(mapping = aes(ymin = conf.low, ymax = conf.high), alpha = 0.4) +
  geom_line() +
 
  xlab("NPOC (ppm)") +
  ylab("Total Nitrogen (ppm)") +
  theme_classic(18)

ggsave(NCplot, filename = "~/Desktop/Alfalfa-Terraformation-GH/Task2A/04_analyses/04_Figures/ManuscriptFigs/NCFig.png",
       height = 6, width =8, units = "in")
