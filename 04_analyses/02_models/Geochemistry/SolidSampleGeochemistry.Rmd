...
This model is consistently ugly, but it's ugly because the majority of the data points are zero. 

```{r}
dat1 <- dat %>% filter(Soil != "P" &
                 IC_s <= 0.24) %>% group_by(Soil) %>% summarise(quantile(IC_s, na.rm = T))
```

as you can see, there is essentially no variation between the different soil treatments and IC. The model output that says there is no relationship seems trustworthy.
...
OC <- lmer(TOC_s ~ Soil + Level +
                 (1|Geno)  + (1|table) + (1|batch_STOC),
                data = filter(dat, TOC_s < 0.48))
# 0.48 is the 97.5% quantile- values above this are outliers
```
